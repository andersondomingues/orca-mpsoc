/*
 * Test a ping pong between the ARM and the ORCA RISCV through the Hermes Router.
...
// number of times the message is sent before ending the application
#define REPETITIONS 3
// repetition counter
u32 repetitions_cnt = 0;

// data buffer sent to RISCV
u32 hermes_pkg[] = {0x00000001, 0x00000004, 0x00000000, 0x44444444, 0x55555555, 0x66666666};
// data buffer received from RISCV
...
int main(){
	...
	for (repetitions_cnt=0;repetitions_cnt<REPETITION;repetitions_cnt++){
		// =======================================
		// Send data
		// =======================================
		...
		*/
		...
		for (int i =3; i<PACKET_SIZE; i++){
			...
			cmp &=  (hermes_pkg[i]) == hermes_pkg_in[i] ?  1 : 0;
		}
		if (cmp==1){
			xil_printf("packets matched !!!\n");
		}else{
